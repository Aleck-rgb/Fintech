...
// Типизация для Next.js 16 (params - это Promise)
type Props = {
...
export default async function ArticlePage({ params }: Props) {
  ...
  function formatInline(text: string): string {
    ...
    // 5. Жирный + Курсив (***text***)
    result = result.replace(
      /(\*\*\*|___)(.*?)\1/g,
      '<strong class="font-bold text-foreground"><em>$2</em></strong>'
    )

    // 6. Жирный (**text**)
    result = result.replace(
      /(\*\*|__)(.*?)\1/g,
      '<strong class="font-bold text-foreground">$2</strong>'
    )
  ...
  function parseMarkdown(content: string): string {
    ...
    let i = 0
    // ВОТ ЦИКЛ, КОТОРЫЙ ВЫЗЫВАЛ ОШИБКУ, ЕСЛИ БЫЛ СКОПИРОВАН НЕВЕРНО
